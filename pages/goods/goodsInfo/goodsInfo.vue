<template>
	<view>
		...
		<view class=" padding-air bg-white " style="padding-top: 25upx; ">
			...
			<view v-else class=" flex justify-between " style="font-weight: 550; overflow: hidden;">
				<text style="font-size: 38upx;color: #ff4040;">
					...
					<text
						class="text-gray old-price"
						style="font-size: 28upx;margin: 0upx 10upx;"
						v-if="goods.min_member_price ? goods.min_member_price < goods.max_price : goods.price < goods.max_price"
					>
		...
		<block v-if="goods.mch">
			<view class="let-mchView" @tap="tpmchinfo" :data-id="goods.mch.id">
				...
				<view><button class="cu-btn">进入</button></view>
			</view>
			<view class="kong"></view>
		</block>
		...
		<view class=" " v-else style="padding-bottom: 10upx;">
			...
			<view class="text-gray text-sm  text-center" style="line-height: 120upx;" v-if="comment.length == 0">暂无评论</view>
		</view>

		<view style="height: 120upx;"></view>
		<view class="foot cu-bar bg-white tabbar border shop  ">
			<button class="action " @tap="tohome">
				<view class="cuIcon-album Guoicon-shouye1 text-gray"></view>
				<view class="text-gray">首页</view>
			</button>
			<view class="action text-gray " @tap="tocard">
				<view class="cuIcon-cart Guoicon-gouwuche "></view>
				购物车
			</view>
			<view class="btn-group">
				<button class="cu-btn bg-orange round shadow-blur" @tap="getall" data-type="3">加入购物车</button>
				<button class="cu-btn bg-red round shadow-blur" @tap="getall" data-type="2">立即购买</button>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		tpmchinfo(e) {
			uni.navigateTo({
				url: '/pages/mch/mchinfo/mchinfo?mch_id=' + e.currentTarget.dataset.id
			});
		},
		getinfo() {
			...
			this.$request.request({
				...
				success(res) {
					...
					if (page == 1) {
						comment = list;
						page = 2;
						comment.length == 0 ? (nolist = true) : (nolist = false);
