		...
		<view class="card-vi">
			...
			<view class="bg-white padding-air" style="padding-bottom: 30upx;padding-top: 20upx;">
				<view class="flex justify-between text-gray align-center " style="height: 70upx;">
					<text>运费</text>
					<text class="text-price">{{ order.express_price }}</text>
				</view>
				<view class="text-gray" style="line-height: 70upx;">
					<text>备注</text>
					<view style="padding: 0upx 20upx; background:RGB(245,245,245); "><input class="textatea" placeholder="请填写备注" @input="getconten" /></view>
				</view>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		zpay() {
			var dzmoney = 0;
			var zmoney = that.new_total_price;
			var goods_info = that.order.goods_info;
			var paytypei = goods_info.type;
			var paytype1 = '';
			if (paytypei == 'ONLY_BUY') {
				paytype1 = 'ONLY_BUY';
			} else {
				paytype1 = goods_info.parent_id != 0 ? 'GROUP_BUY_C' : 'GROUP_BUY';
			}
			goods_info['real_price'] = that.new_total_price;
			goods_info['pay_price'] = dzmoney < zmoney ? (Number(that.new_total_price) - Number(dzmoney)).toFixed(2) : that.new_total_price;
			that.$request.request({
				url: that.api.group.submit,
				data: {
					offline: 1,
				...
				success(res) {
					...
					that.$pay.grouppay(that.paylist[that.payaction].type, res.order_id).then(res1 => {
						if (res1.code == 1) {
							console.log('支付失败！');
							that.$alert('支付失败！');
						} else {
